...
// ---------------------------------------------------------------------------
// doAnalyze — additional coverage (issue #84)
// ---------------------------------------------------------------------------

// TestCoverage_doAnalyze_sdkNotAvailable verifies that doAnalyze returns
// (nil, nil) when the SDK has not been initialised (no hook set, available=false).
...
// when planContent is empty — callers decide whether to gate on emptiness.
...
// context is forwarded to the hook and the hook can use it for early exit.
...
// retry loop in AnalyzeCompletion where the first doAnalyze call fails but
// the second succeeds.
...
// that cancelling the context while waiting in the retry delay causes the
// function to return (nil, nil) instead of continuing.
func TestCoverage_AnalyzeCompletion_contextCancelledDuringRetryDelay(t *testing.T) {
	...
	// Use a context with a very short timeout so it expires during the
	// retry delay (searchRetryDelay = 500ms).
	ctx, cancel := context.WithTimeout(context.Background(), 50*time.Millisecond)
	defer cancel()

	result, err := c.AnalyzeCompletion(ctx, "sess-1", "plan")
	// Context cancellation during retry returns (nil, nil).
	if err != nil {
		t.Errorf("expected nil error, got: %v", err)
	}
	if result != nil {
...
// TestCoverage_AnalyzeCompletion_transportErrorInitRetry exercises the path
// where Init returns a transport error, triggering resetSDK + retry init.
func TestCoverage_AnalyzeCompletion_transportErrorInitRetry(t *testing.T) {
	...
	c.hooks = &testHooks{
		doInit: func(_ context.Context) error {
			...
			if n == 1 {
				// First call: return a transport error to trigger the
				// resetSDK + retry-init path.
				return fmt.Errorf("file already closed")
			}
			// Second call (after reset): succeed.
			return nil
		},
		doAnalyze: func(_ context.Context, _, _ string) (*CompletionAnalysis, error) {
...
// that when Init returns a transport error and the retry-init also fails,
// AnalyzeCompletion degrades gracefully with (nil, nil).
func TestCoverage_AnalyzeCompletion_transportErrorInitRetryFails(t *testing.T) {
	t.Parallel()
	c := New(nil)
	c.hooks = &testHooks{
		doInit: func(_ context.Context) error {
			// Always return a transport error — even the retry fails.
			return fmt.Errorf("file already closed")
		},
	}
	result, err := c.AnalyzeCompletion(context.Background(), "sess-1", "plan")
...
// the context-check between Init and doAnalyze (lines 516-519).
func TestCoverage_AnalyzeCompletion_contextCancelledAfterInit(t *testing.T) {
	...
	c.hooks = &testHooks{
		doInit: func(_ context.Context) error {
			// Cancel the caller context after init succeeds.
			cancel()
			return nil
		},
		doAnalyze: func(_ context.Context, _, _ string) (*CompletionAnalysis, error) {
			t.Error("doAnalyze should not be called when context is cancelled after init")
			return nil, nil
		},
	}
	result, err := c.AnalyzeCompletion(ctx, "sess-1", "plan")
...
// the post-doAnalyze context check (lines 525-529) discards results
// when the context is cancelled during the analysis.
func TestCoverage_AnalyzeCompletion_contextCancelledAfterDoAnalyze(t *testing.T) {
	...
	c.hooks = &testHooks{
		doInit: func(_ context.Context) error { return nil },
		doAnalyze: func(_ context.Context, _, _ string) (*CompletionAnalysis, error) {
			// Cancel the caller context while doAnalyze "succeeds".
			cancel()
	...
	if result != nil {
		t.Error("expected nil result — results should be discarded when context is cancelled")
	}
}

// TestCoverage_AnalyzeCompletion_retryInitFails verifies that when
// doAnalyze fails and the retry-loop reinit also fails, the error
// accumulates and eventually returns "unavailable after N retries".
func TestCoverage_AnalyzeCompletion_retryInitFails(t *testing.T) {
	...
	c.hooks = &testHooks{
		doInit: func(_ context.Context) error {
			n := atomic.AddInt32(&initCalls, 1)
			if n == 1 {
				// First init succeeds to enter the retry loop.
				return nil
			}
			// All subsequent inits during retries fail.
...
// TestCoverage_AnalyzeCompletion_hookReturnsUnparsableResult checks that
// when the hook returns a result directly (not JSON text), it flows through
// AnalyzeCompletion unchanged — the hook bypasses parseCompletionAnalysis.
func TestCoverage_AnalyzeCompletion_hookReturnsUnparsableResult(t *testing.T) {
	t.Parallel()
	c := New(nil)
	c.hooks = &testHooks{
		doInit: func(_ context.Context) error { return nil },
		doAnalyze: func(_ context.Context, _, _ string) (*CompletionAnalysis, error) {
			// Return a result with unusual values — tests that the hook
			// result is passed through without re-parsing.
			return &CompletionAnalysis{
				Complete:       false,
				TotalTasks:     999,
				CompletedTasks: -1,
...
// combined with a short caller-context deadline results in the context
// cancellation path returning (nil, nil).
func TestCoverage_AnalyzeCompletion_slowHookTimeout(t *testing.T) {
	...
	result, err := c.AnalyzeCompletion(ctx, "sess-1", "plan")
	// The caller context expires; AnalyzeCompletion discards and returns (nil, nil).
	if err != nil {
		t.Errorf("expected nil error, got: %v", err)
	}
	if result != nil {
...
// exercises the context check between reinit and the retry doAnalyze call
// (lines 563-565).
func TestCoverage_AnalyzeCompletion_contextCancelledDuringRetryAfterReinit(t *testing.T) {
	...
	c.hooks = &testHooks{
		doInit: func(_ context.Context) error {
			n := atomic.AddInt32(&initCalls, 1)
			if n > 1 {
				// Cancel context during the retry reinit so the
				// post-reinit context check fires.
				cancel()
			}
			return nil
		},
