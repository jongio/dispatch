...
func TestHandleSessionsChanged_LoadingIndicator(t *testing.T) {
	tests := []struct {
		name             string
		initiallyLoading bool
		wantLoading      bool
	}{
		{name: "background refresh stays silent"},
		{name: "foreground indicator is preserved", initiallyLoading: true, wantLoading: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m.dbWatchCh = make(chan struct{}, 1)
			m.sessionsLoading = tt.initiallyLoading

			got, cmd := m.handleSessionsChanged()

			if cmd == nil {
				t.Fatal("handleSessionsChanged returned nil command")
			}
			if got.sessionsLoading != tt.wantLoading {
				t.Fatalf("sessionsLoading = %v, want %v", got.sessionsLoading, tt.wantLoading)
			}
		})
	}
}
