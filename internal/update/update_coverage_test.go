...
func setMockTransport(t *testing.T, fn func(req *http.Request) (*http.Response, error)) {
	t.Helper()
	origTransport := updateHTTPTransport
	origClientTransport := sharedClient.Transport
	mock := &mockRoundTripper{fn: fn}
	updateHTTPTransport = mock
	sharedClient.Transport = mock
	t.Cleanup(func() {
		updateHTTPTransport = origTransport
		sharedClient.Transport = origClientTransport
	})
}
