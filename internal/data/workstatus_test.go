...
// ---------------------------------------------------------------------------
// Continuation plan feedback loop prevention
// ---------------------------------------------------------------------------

func TestParsePlanTasks_IgnoresAutoGeneratedSection(t *testing.T) {
	t.Parallel()

	// Plan with original tasks + auto-generated remaining section.
	content := `# Plan
...
## Remaining Work (auto-generated by dispatch)
...
Summary: 1/3 tasks complete
`
	total, done := ParsePlanTasks(content)
	// Only the original 3 checkboxes should be counted, not the
	// duplicates inside the auto-generated section.
	if total != 3 || done != 1 {
		t.Errorf("ParsePlanTasks() = (%d, %d), want (3, 1)", total, done)
	}
}

func TestParsePlanRemainingItems_IgnoresAutoGeneratedSection(t *testing.T) {
	t.Parallel()
...
Summary: 1/3 tasks complete
`
	got := ParsePlanRemainingItems(content)
	want := []string{"implement feature", "write tests"}
	if !slicesEqual(got, want) {
		t.Errorf("ParsePlanRemainingItems() = %v, want %v", got, want)
	}
}

func TestScanWorkStatus_RescanAfterContinuationPlanStable(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("DISPATCH_SESSION_STATE", dir)

	// Create a session with incomplete tasks and scan.
	writePlan(t, dir, "feedback-loop",
		"# Plan\n\n- [x] setup\n- [ ] implement feature\n- [ ] write tests\n")

	r1 := ScanWorkStatus([]string{"feedback-loop"}, nil)["feedback-loop"]
	if r1.TotalTasks != 3 || r1.DoneTasks != 1 {
		t.Fatalf("initial scan: tasks = %d/%d, want 1/3", r1.DoneTasks, r1.TotalTasks)
	}

	// Write continuation plan.
	if err := WriteContinuationPlan("feedback-loop", r1.RemainingItems, r1.Detail); err != nil {
		t.Fatalf("WriteContinuationPlan: %v", err)
	}

	// Re-scan — counts must not change despite the auto-generated section.
	r2 := ScanWorkStatus([]string{"feedback-loop"}, nil)["feedback-loop"]
	if r2.TotalTasks != r1.TotalTasks || r2.DoneTasks != r1.DoneTasks {
		t.Errorf("re-scan after continuation: tasks = %d/%d, want %d/%d (unchanged)",
			r2.DoneTasks, r2.TotalTasks, r1.DoneTasks, r1.TotalTasks)
	}
	if r2.Status != WorkStatusIncomplete {
		t.Errorf("re-scan status = %v, want WorkStatusIncomplete", r2.Status)
	}
}

func TestStripAutoGeneratedSection(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:    "no auto-generated section",
			content: "# Plan\n\n- [ ] task\n",
			want:    "# Plan\n\n- [ ] task\n",
		},
		{
		...
		{
			name:    "section in middle with trailing content",
			content: "# Plan\n\n- [x] done\n\n## Remaining Work (auto-generated by dispatch)\n\n- [ ] task\n\n## Notes\n\nSome notes.\n",
			want:    "# Plan\n\n- [x] done\n\n## Notes\n\nSome notes.\n",
		},
		{
			name:    "only auto-generated section",
			content: "## Remaining Work (auto-generated by dispatch)\n\n- [ ] task\n",
			want:    "",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			got := stripAutoGeneratedSection(tt.content)
			if got != tt.want {
				t.Errorf("stripAutoGeneratedSection() = %q, want %q", got, tt.want)
			}
		})
	}
}
