...
func ParsePlanTasks(content string) (total int, done int) {
	...
	// Strip the auto-generated "Remaining Work" section so that
	// dispatch-written continuation items are not counted as pending
	// tasks (prevents the feedback loop where re-scanning inflates counts).
	content = stripAutoGeneratedSection(content)
...
func ParsePlanRemainingItems(content string) []string {
	...
	// Strip the auto-generated "Remaining Work" section so that
	// dispatch-written continuation items are not counted as remaining
	// tasks (prevents a feedback loop where re-scanning inflates counts).
	content = stripAutoGeneratedSection(content)
...
// stripAutoGeneratedSection removes the auto-generated "Remaining Work"
// section from plan content. The section spans from remainingWorkHeader
// to the next heading of equal or higher level (## or #), or to EOF.
// This prevents a feedback loop where previously written continuation
// items would be re-counted as pending tasks on the next scan.
func stripAutoGeneratedSection(content string) string {
	idx := strings.Index(content, remainingWorkHeader)
	if idx < 0 {
		return content
	}

	before := content[:idx]
	rest := content[idx+len(remainingWorkHeader):]

	// Find the end of the section: next heading of level 1-2, or EOF.
	endOffset := len(rest)
