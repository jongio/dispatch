...
func runOpen(w io.Writer, args []string) error {
	...
	id, modeFlag, last, printCmd, ov, err := parseOpenArgs(args)
	if err != nil {
		return err
	}
...
// flag, the --print flag, and any per-launch agent/model/yolo overrides from
// the "open" subcommand arguments. args[0] is expected to be "open".
func parseOpenArgs(args []string) (id, mode string, last, printCmd bool, ov launchOverrides, err error) {
	rest := args
	if len(rest) > 0 {
	...
	for i := 0; i < len(rest); i++ {
		...
		if matched, next, mErr := matchLaunchOverride(rest, i, &ov); matched {
			if mErr != nil {
				return "", "", false, false, launchOverrides{}, mErr
			}
			i = next
			continue
		}
		...
		case arg == "--mode" || arg == "-m":
			if i+1 >= len(rest) {
				return "", "", false, false, launchOverrides{}, errors.New("--mode requires a value: inplace, tab, window, or pane")
			}
			mode = rest[i+1]
			i++
		...
		case strings.HasPrefix(arg, "-"):
			return "", "", false, false, launchOverrides{}, fmt.Errorf("unknown flag: %s", arg)
		default:
	...
	if last {
		if len(positionals) > 0 {
			return "", "", false, false, launchOverrides{}, errors.New("open --last does not take a session ID")
		}
	} else {
		switch len(positionals) {
		case 0:
			return "", "", false, false, launchOverrides{}, errors.New("open requires a session ID (or use --last)")
		case 1:
			id = positionals[0]
		default:
			return "", "", false, false, launchOverrides{}, fmt.Errorf("open accepts a single session ID, got %d arguments", len(positionals))
		}
	}
	...
	return id, mode, last, printCmd, ov, nil
}
