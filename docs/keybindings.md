...
14. **Shift+↑** → Extend Selection Upward
    - File: internal\tui\keys.go (ShiftUp binding)
    - Code: key.NewBinding(key.WithKeys("shift+up"))
    - Handler: internal\tui\model.go (shift-arrow handler)
...
15. **Shift+↓** → Extend Selection Downward
    - File: internal\tui\keys.go (ShiftDown binding)
...
17. **s** → Cycle Sort Field
    ...
    - Handler: internal\tui\model.go
    - Behavior: Cycles through sort options (updated, folder, name, attention). Affects session ordering within groups. Folder/repo/branch/host group labels remain A-Z; date group labels follow the selected sort direction.
    - Condition: In session list view
...
18. **S** (Shift+S) → Toggle Sort Direction
    ...
    - Code: key.NewBinding(key.WithKeys("S"))
    - Handler: internal\tui\model.go
    - Behavior: Toggles between ascending and descending sort direction for sessions within groups. In date group mode, it also reverses the date group labels.
    - Condition: In session list view
...
20d. **P** (Shift+P) → Cycle Preview Position
     ...
     - Handler: internal\tui\model.go (cyclePreviewPosition)
...
27b. **c** → Copy Session ID to Clipboard
     ...
     - Handler: internal\tui\model.go (handleCopyID)
...
29. **N** (Shift+N) → Resume All Interrupted Sessions
    - File: internal\tui\keys.go
    - Code: key.NewBinding(key.WithKeys("N"))
    - Handler: internal\tui\model.go (handleResumeInterrupted)
...
71b. **Click on Session ID Row** → Copy Session ID to Clipboard
     - Handler: internal\tui\model.go (handleMouse → HitSessionID)
