...
| T9 | AC4 | Focus keybinding calls platform focus with the session's PID | blocked(requires real Windows terminal; platform code isolated) |
| T10 | AC4 | Focus is no-op when the session's lock file yields no PID | automated |
| T11 | AC4 | Focus is no-op when the located PID is dead | automated |
...
| T14 | AC4 | IsProcessAlive rejects non-positive and nonexistent PIDs | automated |

## Test Mapping

- `internal/data/eventwatcher_test.go`: T1, T2, T3, T4, T13
- `internal/platform/process_test.go`: T10, T11, T14
